...
# Endpoint to get questions based on category. 
  ...
  def quiz_game():
    ...
    try:
      category_id = int(play_category['id'])
      previous_questions = ~Question.id.in_(play_previous)
      ...
      if category_id == 0:
        ...
        questions = (
                      Question.query.filter(previous_questions).filter(Question.category == category_id).all()
                    )
